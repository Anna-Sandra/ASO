...
import { decodeJwtPayload } from "utils/authJwt";
import { h } from "utils/h";
import { apiFetch } from "services/api";
import { apiErrorMessage } from "pages/auth/screensAuth";
import { Button, Field, InlineNotice, OtpCodeInput, TextInput } from "components/ui";
  ...
  return h(
    ...
    [
      ...
      h(
        ...
        h(Link, { to: "/", className: "text-sky-600 hover:underline dark:text-sky-300" }, "← Back to SHOPIQGH")
      )
    ]
  );
}
        ...
        [
          ...
          !loginOtpEmailSent || loginOtpEmailError
            ? h(
                ...
                loginOtpEmailError
                  ? `We could not send the sign-in email: ${loginOtpEmailError} Use “Need a new code?” below or set BREVO_API_KEY + verified Gmail sender on Render.`
                  : "Email is not configured. Set BREVO_API_KEY on the API host (Brevo → Senders, verify Gmail)."
              )
            : null,
          showResendCard
